...
export function RegisterPage() {
  ...
  const {
    ...
    isError,
    refetch,
  } = useQuery<GetUserByInviteIdResponse>({
    queryKey: ["users:create:invite", inviteId],
    queryFn: () =>
      getUserByInviteId({
        inviteId,
      }),
  ...
  async function handleSubmitForm(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      setLoadingForm(true);
      await api.post(`/users/invite/${inviteId}`, {
        password,
    ...
    try {
      ...
      await api.post(`/users/invite/${inviteId}`, {
        individual: {
          ...
          documentIssuing: documentExpedido,
          documentDateIssue: documentDateIssueFormattedPrepated,
          motherName,
  ...
  return (
    <>
      <div className="flex min-h-screen flex-1 flex-col items-center p-3 sm:p-6 lg:p-8 bg-white">
                    ...
                    <div className="grid gap-2">
                      {isLoading ? (
                        ...
                        <>
                          ...
                          <Input
                            id="last-name"
                            value={Format.CPF(result?.user.document)}
                            disabled
                          />
                        </>
                      )}
        ...
        {result?.user?.status === "WAITING_INDIVIDUAL" && (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="grid gap-2">
                      {isLoading ? (
                        ...
                        <>
                          <Label htmlFor="document-uf">UF da expedição</Label>
                          <Input
                            id="document-uf"
                            placeholder="SP"
                            value={documentUF}
                            maxLength={2}
                            onChange={(e) => setDocumentUF(e.target.value)}
                          />
                        </>
                      )}
                    </div>
        ...
        {result?.user?.status === "WAITING_ADDRESS" && (
        ...
        {result?.user?.status === "WAITING_DOCUMENT" && (
          <Card className="mx-auto max-w-md lg:w-full">
            <CardHeader>
                ...
                <div className="grid gap-4">
                  <div className="grid gap-2">
                    {isLoading ? (
                      ...
                      <>
                        <Label htmlFor="selfie">Foto (Selfie)</Label>
                        <Input
                          id="selfie"
                          type="file"
                          onChange={handleSubmitFormDocumentsSelfie}
                        />
                      </>
                    )}
                  </div>
                  <div className="grid gap-2">
                    {isLoading ? (
                      ...
                      <>
                        <Label htmlFor="front">CNH ou RG (Frente)</Label>
                        <Input
                          id="front"
                          type="file"
                          onChange={handleSubmitFormDocumentsFront}
                        />
                      </>
                    )}
                  </div>
                  ...
                  <div className="grid gap-2">
                    {isLoading ? (
                      ...
                      <>
                        <Label htmlFor="verse">CNH ou RG (Verso)</Label>
                        <Input
                          id="verse"
                          type="file"
                          onChange={handleSubmitFormDocumentsVerse}
                        />
                      </>
                    )}
                  </div>
                  ...
                  >
                    {loadingForm ? (
                      ...
                      "Anexe os documentos para concluir"
                    )}
                  </Button>
                </div>
              </form>
        ...
        {(result?.user?.status === "WAITING_ANALYSIS" ||
          result?.user?.status === "IN_ANALYSIS" ||
          result?.user?.status === "APPROVED" ||
          result?.user?.status === "PRE_APPROVED") && (
          <Card className="mx-auto max-w-md lg:w-full">
            <CardHeader>
        ...
        {(result?.user?.status === "REJECTED" ||
          result?.user?.status === "RECEJECTED_KYC") && (
          <Card className="mx-auto max-w-md lg:w-full">
            <CardHeader>
